...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    logRouteError('admin_users_proxy_error', err, traceId, { session_trace_id: sessionTraceId });
    return jsonWithTrace({ error: 'Failed to fetch admin data' }, { status: 500 }, traceId, sessionTraceId);
  }
}
